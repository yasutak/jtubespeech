...
from util import get_subtitle_language, make_video_url
...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="filename of list checkpoint (for restart retrieving)"
    )
    ...
    for videoid in tqdm(open(fn_videoid).readlines()):
        ...
        try:
            result = subprocess.check_output(
                "yt-dlp --list-subs --sub-lang {lang} --skip-download {url}", shell=True, universal_newlines=True
            )
            auto_lang, manu_lang = get_subtitle_language(result)
            subtitle_exists = subtitle_exists.append(
                {"videoid": videoid, "auto": lang in auto_lang, "sub": lang in manu_lang}, ignore_index=True
            )
            n_video += 1
        except:
            pass
...
if __name__ == "__main__":
    ...
    filename = retrieve_subtitle_exists(args.lang, args.videoidlist, args.outdir, fn_checkpoint=args.checkpoint)
