...
from util import autovtt2txt, make_basename, make_video_url, vtt2txt
...
def download_video(lang, fn_sub, outdir="video", wait_sec=10, keep_org=False):
    ...
    for videoid in tqdm(sub[sub["sub"] == True]["videoid"]):  # manual subtitle only
        ...
        if not fn["wav16k"].exists() or not fn["txt"].exists():
            ...
            cp = subprocess.run(
                f"yt-dlp --sub-lang {lang} --extract-audio --audio-format wav --write-sub {url} -o {base}.\%\(ext\)s --concurrent-fragments {args.threads}",
                shell=True,
