...
def parse_args():
    ...
    parser.add_argument("--wait", type=float, default=0.2, help="waiting time between queries")
    ...
    with open(fn_videoid, "w") as f:
        for word in tqdm(list(open(fn_word, "r").readlines())):
            try:
                ...
                # find video IDs
                videoids_found = [
                    x.split(":")[1].strip('"').strip(" ") for x in re.findall(r"\"videoId\":\"[\w\_\-]+?\"", str(html))
                ]
...
if __name__ == "__main__":
    ...
    filename = obtain_video_id(args.lang, args.wordlist, args.outdir, args.wait)
